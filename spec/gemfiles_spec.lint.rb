  ...
  shared_examples_for "a sane gemfile" do |gemfile|
    it "is up to date" do
      ...
      new_lockfile = Bundler.with_original_env do
        `BUNDLE_GITHUB__HTTPS=true BUNDLE_GEMFILE=#{gemfile} bundle lock --print`
      end

      msg = "Please update #{gemfile}'s lock file with `BUNDLE_GITHUB__HTTPS=true BUNDLE_GEMFILE=#{gemfile} bundle install` and commit the result"
